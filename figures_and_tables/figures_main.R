...
pdf(file = here('figures_and_tables','figure1.pdf'),width=5.5,height=2.5,pointsize=1)
par(mfrow=c(1,3)) 
plot(win,xlim=c(128,143),ylim=c(33,41),lwd=0.5,border='darkgrey',col='darkgrey',bg='powderblue',main='a',cex.main=1.5)
...
plot(win,xlim=c(-8,1),ylim=c(50,61),border='darkgrey',col='darkgrey',bg='powderblue',main='b',cex.main=1.5)
...
plot(win,xlim=c(-8,1),ylim=c(50,61),border='darkgrey',col='darkgrey',bg='powderblue',main='c',cex.main=1.5)
abline(v=seq(-10,4,2),h=seq(48,64,2),col='grey77',lwd=0.5)
plot(gb,col='cornsilk',add=T,border=NA)
# plot(gb.abot.sites,pch=1,add=T,cex=0.8)
plot(gb.burial.sites,pch=1,add=T,cex=0.8)
axis(2,tck=-0.03,padj=-0.5,at=seq(48,64,4))
axis(2,tck=-0.01,at=seq(48,64,2),labels=NA)
axis(1,tck=-0.03,padj=-0.5,at=seq(-12,12,4))
axis(1,tck=-0.02,at=seq(-12,12,2),labels=NA)
...
legend(x=6000,y=0.85,legend=c('90% Prediction Envelope','Positive Deviation','Negative Deviation'),fill=c('lightgrey','red','blue'),bty='n')
