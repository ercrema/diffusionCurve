...
constants$cra_error  <- rep(20,constants$N)

# Simulate Response Variable
adoptionModel.sim  <- nimbleModel(adoptionModel,constants=constants)
adoptionModel.sim$theta  <- round(runif(constants$N,min=1750,max=5000))
adoptionModel.sim$simulate('mu')
adoptionModel.sim$simulate('sigmaCurve')
adoptionModel.sim$simulate('sigma')
adoptionModel.sim$simulate('cra')
adoptionModel.sim$r  <- r
adoptionModel.sim$m  <- m
adoptionModel.sim$mu_k  <- mu_k
...
d$cra  <- round(adoptionModel.sim$cra)
...
inits$theta  <- medCal(calibrate(d$cra,constants$cra_error))
