...
# Data Preparation ----
ricedata <- subset(ricedata,!is.na(Region2))
...
ricedata$Region3  <- as.integer(factor(ricedata$Region2,levels=c('N.Kyushu','Kinki','S.Kanto','Tohoku')))
site.region  <- unique(data.frame(ricedata$site.id,ricedata$Region3))
...
constants$region  <- ricedata$Region3
...
runFun  <- function(seed, d, constants, theta, init, nburnin, niter, thin)
{
	...
	adoptionModel  <- nimbleCode({
		...
		r[4] ~ dexp(100) # prior adoption rate
		...
		m[4] ~ T(dnorm(mean=2500,sd=500),1000,50000) #prior mid-point
		...
		mu_k[3] ~ dnorm(0,1) #hyperprior for site prior for region 3
		mu_k[4] ~ dnorm(0,1) #hyperprior for site prior for region 4
		sigma_k ~ dinvgamma(5,5) #hyperprior for site prior
	})

	#Define inits
	inits  <- list()
	inits$r  <- c(0.0001,0.0001,0.0001,0.0001)
	inits$m  <- c(3000,3000,3000,3000)
	inits$mu_k  <- c(1,1,1,1)
	inits$sigma_k  <- 1
...
post.sample.core.jpregional2  <- post.sample.combined[,!grepl('theta|logk',colnames(post.sample.combined))]
...
adoptionSimModel  <- nimbleCode({
	...
	mu_k[4] ~ dnorm(0,1) #hyperprior for site prior for region 2
	sigma_k ~ dinvgamma(5,5) #hyperprior for site prior
})
...
ppmat.jpregional2  <- ppmat
constants.jpregional2  <- constants
d.jpregional2  <- d

# Store output ----
save(constants.jpregional2,d.jpregional2,ppmat.jpregional2,file=here('results','ppcheck_jpregional2.RData'))
save(rhats,post.sample.core.jpregional2,file=here('results','post_jpregional2.RData'))
