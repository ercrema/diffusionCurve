...
ricedata$Region3  <- 1
ricedata$Region3[which(ricedata$Region%in%c('Chubu','Kanto'))]  <- 2
ricedata$Region3[which(ricedata$Region%in%c('Tohoku'))]  <- 3
...
post.sample.core.jpregional  <- post.sample.combined[,!grepl('theta|logk',colnames(post.sample.combined))]
