...
runFun  <- function(seed, d, constants, theta, init, nburnin, niter, thin)
{
	...
	inits$mu_k  <- 0.7
	inits$sigma_k  <- 1
	inits$k  <- rbeta(constants$NSites,(inits$mu_k*inits$sigma_k +1),((1-inits$mu_k)*(sigma_k)+1))
	inits$theta  <- theta
...
save(constants.jp.abot,d.jp.abot,ppmat.jp.abot,file=here('results','ppcheck_jp_abot2.RData'))
save(rhats.jp.abot,post.sample.core.jp.abot,file=here('results','post_jp_abot2.RData'))
