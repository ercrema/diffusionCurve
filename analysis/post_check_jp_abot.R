...
fitted.mat  <- apply(ppmat.params.jp.abot,1,function(x){sigmoid(4000:1700,r=x[1],m=x[2],k=x[3])})
fitted.mean.jp.abot  <- apply(fitted.mat,1,mean)
fitted.90env.jp.abot  <- apply(fitted.mat,1,quantile,prob=c(0.1,0.9))

ppc.jp.abot  <- ppcheck(caldates.jp.abot,obs.bin.jp.abot,ppmat.jp.abot,timeRange=c(4000,1700))

save(ppc.jp.abot,fitted.mean.jp.abot,fitted.90env.jp.abot,file=here('results','ppc_jp_abot.RData'))
